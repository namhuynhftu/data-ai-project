...
product_reviews as (
    ...
    from {{ ref('fact_order_items') }} fi
    inner join {{ ref('fact_reviews') }} r on fi.order_id = r.order_id
    group by 1
),
...
from product_dim pd
inner join product_sales ps on pd.product_key = ps.product_key
left join product_reviews pr on pd.product_key = pr.product_key
